...
eval:
	...
	if(input->this.pair.car->type==SFS_SYMBOL)
	{
	    ...
	    if(!strcmp(input->this.pair.car->this.symbol,"IF") ||
	       !strcmp(input->this.pair.car->this.symbol,"AND") ||
	       !strcmp(input->this.pair.car->this.symbol,"DEFINE") ||
	       !strcmp(input->this.pair.car->this.symbol,"OR") ||
	       !strcmp(input->this.pair.car->this.symbol,"SET!") ||
	       !strcmp(input->this.pair.car->this.symbol,"QUOTE"))
	    {
		...
		while(circulation->type!=SFS_NIL)
		{
		    ...
		    if(circulation->this.pair.car->this.pair.cdr->this.pair.car->type!=SFS_NUMBER &&
		       circulation->this.pair.car->this.pair.cdr->this.pair.car->type!=SFS_SYMBOL)
		    {
			WARNING_MSG("Erreur de syntaxe du let!");
			return input;
		    } 
	...
	if (strcmp(input->this.symbol,"+")==0 ||
	    strcmp(input->this.symbol,"-")==0 ||
	    strcmp(input->this.symbol,"*")==0 ||
	    strcmp(input->this.symbol,"/")==0 )
	{
